...
function Planet ({ id, cellCount, controlledBy, color='#FFFFFF', selectedBy = null, x, y}) {
    ...
    function getId () {
      ...
      function attack (bulletPlanetID, planetCellCount, bulletColor, targetStrength, planetStrength){
        ...
        console.log("planetStrength " + planetStrength);
        console.log(" targetStrength " + targetStrength);
          console.log("planet.getControlledBy() " + bulletPlanetID);
          console.log("controlledBy " + controlledBy);
        if(bulletPlanetID === controlledBy){
          console.log("id " + id);
          console.log("bulletPlanetID " + bulletPlanetID);

          if(bulletPlanetID !== id){
            cellCount +=  planetCellCount*(targetStrength/planetStrength);
          }
        } else {
          cellCount += -planetCellCount*(targetStrength/planetStrength);
          if(cellCount < 0){
